...
Base invitation mengetahui metadata aplikasi stabil dan content object opaque. `invitation-categories/` menyatakan kapabilitas semua modul secara eksplisit. `invitation-modules/definitions/` memiliki schema/default/editor capability/migrasi semantik; core content mengelola envelope v2. Paket `templates/<key>/` memiliki manifest, renderer, section declaration, theme policy, dan preset. Generator build-time memindai paket bernama aman yang lengkap lalu menghasilkan import statis; tidak ada runtime filesystem lookup, `eval`, remote executable loading, atau manual registry import. Template mengonsumsi modul tanpa memiliki schema bisnis. `themes/` dan `invitation-design/` memvalidasi override, allowlist asset/font, dan fallback.

Template baru yang hanya memakai module existing cukup menambah paket `templates/<key>/`, menjalankan generator/check, dan—bila tersedia untuk create/switch—menambah entry katalog melalui migration forward-only. Application service, repository, route, editor router, dan adapter Supabase tidak mengenumerasi template. Template ID yang sudah masuk production tidak boleh di-rename; hanya display name yang boleh berubah.
