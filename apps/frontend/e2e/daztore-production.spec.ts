...
test.describe("daztore-inv1 production browser journey", () => {
  ...
  test("Create through moderation remains usable and resilient", async ({ page }, testInfo: TestInfo) => {
    ...
    await expect(page.getByLabel("Template")).toHaveValue("daztore-inv1@1");
    ...
    await expect(guestPage.locator('[data-template="daztore-inv1@1"]')).toHaveAttribute("data-opened", "true");
    ...
    const rsvp = guestPage.locator("#daztore-rsvp");
    ...
    const wishes = guestPage.locator("#daztore-wishes");
    ...
    await expect(deniedPage.locator('[data-template="daztore-inv1@1"]')).toHaveAttribute("data-opened", "true");
    ...
    await guestPage.getByRole("button", { name: "Buka Undangan" }).click();
    await guestPage.locator("#daztore-wishes").scrollIntoViewIfNeeded();
    ...
    await guestPage.goto(`/${slug}`);
    await expect(guestPage.getByRole("button", { name: "Buka Undangan" })).toHaveCount(0);
