...
describe("daztore wedding composition", () => {
  ...
  it("hydrates newly added optional modules without deleting existing inactive data", () => {
    ...
    const migrated = parseTemplateContent("daztore-inv1", 1, 2, oldV2);
  ...
  it("maps both Daztore presets into the complete semantic CSS variable set", () => {
    const first = themeCssVariables(getRegisteredTheme("daztore-inv1-default", 1)!);
    const second = themeCssVariables(getRegisteredTheme("daztore-inv1-blue", 1)!);
