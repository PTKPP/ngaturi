...
export function BottomNavigation({ visible, hasGallery, hasGift }: { visible: boolean; hasGallery: boolean; hasGift: boolean }) {
  const items = useMemo<NavigationItem[]>(() => [
    { id: "wedding-default-home", label: "Home", icon: "home" },
    { id: "wedding-default-couple", label: "Mempelai", icon: "couple" },
    { id: "wedding-default-events", label: "Acara", icon: "calendar" },
    ...(hasGallery ? [{ id: "wedding-default-gallery", label: "Galeri", icon: "photo" as const }] : []),
    ...(hasGift ? [{ id: "wedding-default-gift", label: "Hadiah", icon: "gift" as const }] : []),
  ], [hasGallery, hasGift]);
