...
import { templateId, themeId, type Invitation, type InvitationEvent, type InvitationStatus, type Session } from "@/domain";
import type { DemoRuntime } from "@/lib/demo-runtime";
import { createPrototypeId } from "@/services";
...
function InvitationEditor({ initialInvitation }: { initialInvitation: Invitation }) {
  ...
  const themes = runtime?.themes.listForTemplate(invitation.templateKey, invitation.templateVersion).filter((item) => item.status === "active") ?? [];
  const route = runtime && session ? runtime.routeService.getOwned(session, invitation.routeId) : null;
  ...
  return <AppShell title="Editor undangan">
    ...
    <form className="form" onSubmit={save}>
      ...
      <section className="panel form" id="tampilan"><h2>Tampilan</h2><label className="field"><span>Template</span><select value={`${invitation.templateKey}@${invitation.templateVersion}`} onChange={(event) => { const [templateKey, rawVersion] = event.target.value.split("@"); const templateVersion = Number(rawVersion); const defaultTheme = runtime?.themes.findDefault(templateKey, templateVersion); if (!defaultTheme) { setError("Tema default aktif tidak tersedia."); return; } setInvitation({ ...invitation, templateKey, templateVersion, themeKey: defaultTheme.key, themeVersion: defaultTheme.version }); }}>{templates.map((template) => <option key={templateId(template)} value={templateId(template)}>{template.name} — {template.description}</option>)}</select><small>Mengganti template mempertahankan konten dan memilih tema default template tujuan.</small></label><label className="field"><span>Tema</span><select value={`${invitation.themeKey}@${invitation.themeVersion}`} onChange={(event) => { const [themeKey, rawVersion] = event.target.value.split("@"); setInvitation({ ...invitation, themeKey, themeVersion: Number(rawVersion) }); }}>{themes.map((theme) => <option key={themeId(theme)} value={themeId(theme)}>{theme.name} — {theme.description}</option>)}</select><small>Tema hanya mengubah token visual, bukan route, konten, atau struktur template.</small></label></section>
