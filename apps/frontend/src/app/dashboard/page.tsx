...
export default async function DashboardPage() {
  ...
  const [usage, routes] = await Promise.all([repository.routeUsage(profile), repository.listOwnedRoutes(profile.id)]);
  return <AppShell title="Dashboard">
    ...
    <div className="grid cards">
      ...
      <article className="card"><h2>Tersimpan aman</h2><p>Data undangan, route, dan status disimpan di Supabase dengan isolasi per owner.</p></article>
    </div>
  </AppShell>;
}
