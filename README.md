...
1. Salin nama variabel dari `.env.example` ke `apps/frontend/.env.local` dan isi URL serta publishable key proyek Supabase. `SUPABASE_SERVICE_ROLE_KEY` dan `GUEST_SUBMISSION_RATE_LIMIT_SECRET` hanya boleh tersedia di server; `RSVP_RATE_LIMIT_SECRET` tetap diterima sebagai fallback kompatibilitas. Reverse proxy production harus menimpa `X-Real-IP`/`X-Forwarded-For`, bukan meneruskan nilai spoofed dari client, agar fingerprint rate limit RSVP/Wishes stabil.
